...
st.markdown(
    """
    **Uplift Agent (PoC)** — агент оценивает эффект заданной клиентской
    интервенции (скидка, апселл, смена тарифа, зарплатный проект и т.п.)
    для конкретного SME-клиента банка (Small and Medium Enterprise — малый
    и средний бизнес) и объясняет ожидаемый результат через
    три независимых источника: PSM (статистика по похожим клиентам), причинный
    граф (LLM + алгоритмические методы) и RAG по внутренним методичкам.
    Система не принимает решение сама, а готовит обоснованную рекомендацию
    для аналитика — с уровнем уверенности и ссылками на использованные данные.

    **Как попробовать:** либо опишите запрос своими словами (можно указать ID
    клиента прямо в тексте), либо выберите клиента и нажмите быструю кнопку с
    готовым сценарием интервенции.
    """
)
