        ...
        self._prompt_whatif = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "• Фокусируйтесь на практических аспектах реализации\n\n"
                    "ПЯТЬ ПРИНЦИПОВ КАЧЕСТВЕННОГО АНАЛИТИЧЕСКОГО ВЫВОДА:\n"
                    "\n"
                    "(A) Числовая величина эффекта берётся ТОЛЬКО из PSM.\n"
                    "RAG-документы могут содержать численные оценки эффектов в других исследованиях/контекстах. "
                    "Эти числа — отраслевой фон, не прогноз для текущего клиента. Когда упоминаете число из RAG, "
                    "обязательно поясняйте, что это контекст другого исследования, а не оценка эффекта данной "
                    "интервенции. Любой количественный ориентир ожидаемого эффекта строится на ATT (или его "
                    "отсутствии), а не на цифре из документа.\n"
                    "\n"
                    "(B) Утверждения о состоянии клиента — только из его профиля.\n"
                    "Любое предположение о фактах клиента (наличие/отсутствие продукта, уровень активности, "
                    "размер бизнеса, базовая метрика) должно прямо опираться на конкретное поле профиля. Если "
                    "поля в профиле нет — не упоминайте этот факт. PSM, RAG и Graph не описывают конкретного "
                    "клиента: PSM описывает среднее по сопоставленным клиентам, RAG — доменный контекст, "
                    "Graph — структурные связи переменных в популяции.\n"
                    "\n"
                    "(C) Утверждения о причинной структуре — только из [GRAPH_DSL].\n"
                    "Отсутствие ребра или цепочки в DSL — это умолчание (нехватка явной информации), а не "
                    "доказательство отсутствия причинной связи. Корректная формулировка для не отражённого "
                    "пути одна: «в предоставленном фрагменте графа этот путь явно не отражён». На этой "
                    "формулировке нельзя строить вывод об отсутствии эффекта или о высоком доверии к нулевому "
                    "эффекту.\n"
                    "\n"
                    "(D) Знак драйверов согласован со знаком интервенции и знаком PSM ATT.\n"
                    "Перед формулированием каждого пункта drivers_pos / drivers_neg проверьте: "
                    "соответствует ли он направлению изменения из intervention delta (увеличение/уменьшение/"
                    "включение/выключение) и знаку PSM ATT. Драйвер, противоречащий направлению самой "
                    "интервенции, недопустим — он переворачивает интерпретацию ожидаемого эффекта.\n"
                    "\n"
                    "(E) Recommendations — не менее двух конкретных действий по интервенции.\n"
                    "Каждая рекомендация должна:\n"
                    "  1) начинаться с глагола действия в инфинитиве;\n"
                    "  2) явно называть саму интервенцию из delta (а не общую тему предметной области);\n"
                    "  3) указывать сегмент клиента или условие запуска через конкретное поле профиля и порог;\n"
                    "  4) содержать измеримый критерий успеха (метрику и направление её изменения).\n"
                    "Общие пожелания вида «учесть», «оценить», «проверить», «адресно предложить» без "
                    "сегмента и критерия — неприемлемы.\n"
                    "\n"
                    "Перед отправкой ответа сверьте каждый пункт с принципами (A)–(E) и переформулируйте "
                    "там, где обнаружите расхождение.\n\n"
