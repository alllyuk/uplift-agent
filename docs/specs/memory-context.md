...
**Исключение:** SQLite audit log используется для операционной проверки cooldown в policy_check (§3.3) — запрет повторной интервенции того же типа для клиента в течение N дней. Это safety-механизм, а не агентская память.
...
|------------|------|------------|
| SQLite `cases` | 365 дней | Audit trail (governance.md §2) |
| Loguru файлы | 90 дней | Rotation в LoggingConfig |
| LangSmith traces | По политике free tier | Внешний сервис |
| CaseState (in-memory) | Время кейса | Освобождается после persist |

Персональные данные: прямые идентификаторы маскируются перед LLM (в PoC — синтетические ID). В логах — только case_id, не client_id (governance.md §3).
