...
### 4.3 Critic Checks Level 1 (structural)

Уровень 1 critic выполняется после каждой генерации LLM. Все проверки детерминированные, без LLM. Это **safety floor** — только структурные проверки на данных, которые можно верифицировать без NLU.

**Check 1 — Атрибуция источников:** если цитируется `doc_id` (паттерн `doc_\d+`), проверяется его наличие в retrieved chunks. Цитирование отсутствующего документа → issue.

**Check 2 — Полнота ответа:** обязательные поля Explanation (`drivers_pos`, `drivers_neg`, `expected_effect`) заполнены и содержат >= 10 символов.

Семантические проверки (числовая консистентность, валидность рёбер графа, адекватность хеджирования) делегированы Level 2 (LLM-critic), т.к. regex-проверки на свободном тексте дают слишком много ложных срабатываний.

### 4.4 Critic Checks Level 2 (LLM-augmented)

Запускается **только** если уровень 1 не нашёл блокирующих issues. LLM-критик проверяет смысловые аспекты. Это часть **гибридной агентности** (см. ADR-8 в `system-design.md`): rule-based — safety floor, LLM — семантический слой.

На **retry-проходе** L2 не запускается — его замечания уже были использованы как фидбэк для повторной генерации. После retry проверяется только L1.
